...
struct ParserTest
{
   ...
   bool ProxyTest(UtPath aTestFile)
   {
      ...
      // First, process the input test making a new file
      {
         ...
         std::ifstream is(aTestFile.GetSystemPath().c_str());
         ...
         std::ofstream os(ppFile.GetSystemPath().c_str());
         os.write(textWithExtras.c_str(), textWithExtras.size());
         std::ofstream clipOs(clippedFile.GetSystemPath().c_str());
      ...
      WsfPProxyValue desiredProxy;
      {
         ...
         desiredProxy = WsfPProxyValue::ConstructNew(mProxyRegistry->GetType("root"));
      ...
      WsfPProxyValue finalProxy = WsfPProxyValue::ConstructNew(mProxyRegistry->GetType("root"));
      ...
      WsfPProxyUndoValue finalUndo(finalProxy);
      {
         ...
         else
         {
            ...
            for (size_t i = 0; i < errors.size(); ++i)
            {
               ...
               {
                  size_t line, col;
                  errors[i]->mRange.mSource->PositionToLineColumn(errors[i]->mRange.GetBegin(), line, col);
...
int main(int argc, char* argv[])
{
   ...
   bool       ok = parseTest.InitParser((dataRoot + "./grammar").GetSystemPath(), dataRoot.GetSystemPath());
   if (!ok)
   {
      return 1;
   }
